...
def box_plot_multiple(data,nb_pt,title,tl=False):
    ...
    if tl == False:
        ...
        box_colors = ['#3B3B3B','#012949','#035CA3', '#088FFA',  '#62B8FC']
    elif tl == True:
            # TL
        #box_colors = ['#526D82',"#C539B4",'#F9F54B','#00FFCA','#539165']
        box_colors = ['#3B3B3B','#6D0E10','#BF181D', '#E84B4F', '#F29C9E']
    ...
    print(pos)
    bp = ax1.scatter(pos,df,c=c,marker='X',s=100)
    ...
    for i in range(num_boxes):
        ...
        box_coords = np.column_stack([pos[i], df[i]])
    ...
    space = 0.035
    fig.text(0.75, 0.12 , f'Expert',
                backgroundcolor=box_colors[0], color='black', weight='roman',
                size='x-small')
    for i in range(nb_pt-1):
        fig.text(0.80, 0.12 - (i * space), f'{nm[i]}',
                backgroundcolor=box_colors[i+1], color='black', weight='roman',
                size='x-small')
...
if __name__ == '__main__':
    ...
    data = []
    data.append(collect_wins(load_data(os.path.join('Expert',os.listdir('Expert')[0] ))))
    for file in files:
        print(file)
        data.append(collect_wins(load_data(os.path.join(folder, file) )))
        print(data)
    ...
    exp_data = collect_wins(load_data(os.path.join('Expert',os.listdir('Expert')[0] )))
