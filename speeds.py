...
def collect_average_speed(data):
    speeds = []
    for i in range(5):
        block_speeds = []
        for j in range(10):
            states = data['block_'+str(i)]['states'][j]
            temp =  []
            for point in states:
                temp.append(math.sqrt(point[2]**2 + point[3]**2))
            block_speeds.append(np.mean(temp))
        speeds.append(block_speeds)
...
def box_plot_multiple(data,nb_pt,title,tl):
    ...
    ax1.set(
        ...
        title=title,
        xlabel='Blocks',
        ylabel='Average Ball Game Speed',
    )
...
if __name__ == '__main__':
    ...
    data = []
    data.append(collect_average_speed(load_data(os.path.join('Expert',os.listdir('Expert')[0] ))))
    for file in files:
        print(file)
        data.append(collect_average_speed(load_data(os.path.join(folder, file) )))
        print(data)
    ...
    exp_data = collect_average_speed(load_data(os.path.join('Expert',os.listdir('Expert')[0] )))
