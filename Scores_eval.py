...
def collect_average_scores(data):
    ...
    scores.append(data['block_pretesting']['step_counter'])
    done.append(data['block_pretesting']['done'])
...
def box_plot_multiple(data,nb_pt,title,tl=False):
    ...
    print(pos)
    bp = ax1.boxplot(data, notch=False, sym='+', vert=True, whis=1,positions=pos)
    ...
    ax1.set_xticklabels(['','','',''], rotation=45, fontsize=8)
    ...
    for i in range(nb_pt):
        fig.text(0.80, 0.12 - (i * space), f'{nm[i]}',
                backgroundcolor=box_colors[i], color='black', weight='roman',
                size='x-small')
...
if __name__ == '__main__':
    ...
    box_plot_multiple(data,4,'No Transfer Learning Group',False)
