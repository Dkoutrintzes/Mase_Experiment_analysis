...
def collect_states(data):
    states = []
    for i in range(5):
        states.append(data['block_'+str(i)]['states'])
    return states
...
def box_plot_multiple(data,nb_pt,title,tl=False):
    ...
    ax1.set(
        ...
        title=title,
        xlabel='Blocks',
        ylabel='Normilize Participant Tray Changes',
    )
...
if __name__ == '__main__':
    ...
    e_states = collect_states(e_data)
    e_inputs = []
    for block in e_states:
        temp_inputs = []
        for game in block:
            inputs = get_inputs(game)
            temp_inputs.append(inputs/len(game))
        e_inputs.append(temp_inputs)
    
    print(e_inputs)
    data = []
    data.append(e_inputs)
    for file in files:
        p_data = load_data(os.path.join(folder,file))
        p_states = collect_states(p_data)
        p_inputs = []
        for block in p_states:
            temp_inputs = []
            for game in block:
                inputs = get_inputs(game)
                temp_inputs.append(inputs/len(game))
            p_inputs.append(temp_inputs)
        data.append(p_inputs)

    print(np.array(data).shape)   
    #print(data) 
    box_plot_multiple(data,5,'Normalized Keyboard Inputs During a Game\nTransfer Learning Group',True)
