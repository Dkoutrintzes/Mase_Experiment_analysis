...
def collect_average_scores(data):
    scores = []
    for i in range(5):
        scores.append(data['block_'+str(i)]['distance_travel'])
    return scores

def collect_average_steps(data):
    scores = []
    for i in range(5):
        scores.append(data['block_'+str(i)]['step_counter'])
    return scores

def get_normilized_paths(data):
    paths = []
    paths = collect_average_scores(data)
    steps = collect_average_steps(data)

    for i in range(len(paths)):
        for j in range(len(paths[i])):
            paths[i][j] = paths[i][j] * (steps[i][j]/200)
    return paths
...
def box_plot_multiple(data,nb_pt,title,tl):
    ...
    ax1.set(
        ...
        title=title,
        xlabel='Blocks',
        ylabel='Normalized Travel Distance',
    )
    ...
    for tick, label in zip(range(num_boxes), ax1.get_xticklabels()):
        k = tick % nb_pt
        ax1.text(pos[tick], .95, "{:.2f}".format(float(upper_labels[tick])),
...
if __name__ == '__main__':
    ...
    data = []
    e_data = load_data(os.path.join('Expert',os.listdir('Expert')[0] ))
    e_normilized_paths = get_normilized_paths(e_data)
    data.append(e_normilized_paths)
    for file in files:
        print(file)
        p_data = load_data(os.path.join(folder, file))
        P_normilized_paths = get_normilized_paths(p_data)

        data.append(P_normilized_paths)
        print(data)
        
    box_plot_multiple(data,5,'Transfer Learning Group',True)
